...
import 'carinfo_model.dart';
export 'carinfo_model.dart';

class CarinfoWidget extends StatefulWidget {
  static const String routName = 'Car_Info Page';
...
class _CarinfoWidgetState extends State<CarinfoWidget> {
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => CarinfoModel());

    _model.textController1 ??= TextEditingController(
        text: CarInfoCubit.get(context).myCarInfo?["model"] ?? "");
    _model.textFieldFocusNode1 ??= FocusNode();

    _model.textController2 ??= TextEditingController(
        text: CarInfoCubit.get(context).myCarInfo?["color"] ?? "");
    _model.textFieldFocusNode2 ??= FocusNode();

    _model.textController3 ??= TextEditingController(
        text: CarInfoCubit.get(context).myCarInfo?["plates"] ?? "");
            ...
            children: [
              ...
              BlocConsumer<CarInfoCubit, CarInfoState>(
                listener: (context, state) {
                  if (state is SuccessEditCarInfoState) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                        content: Text("Your Car Saved Successfully")));
                    Navigator.pop(context);
                  }
                },
                builder: (context, state) {
                  if (state is LoadingEditCarInfoState) {
                  ...
                  return FFButtonWidget(
                    onPressed: () {
                      if (_model.textController1.text.isEmpty ||
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                            content: Text(
                                "Must Enter All fields before save your car")));
                      } else {
                        CarInfoCubit.get(context).editCar(
